<template>
  <el-container v-loading="loading" class="ofa-container column">
    <el-header class="header">
      <span>
        <span class="title">岗位职务<span class="sub-title" style="margin-left: 10px;">共有<label class="ofa-text-primary"> {{list.length}} </label>条数据</span></span>
      </span>
      <span>
        <el-button @click="showAddDialog" type="primary" size="small">
          <font-awesome-icon fas icon="plus"></font-awesome-icon>&nbsp;新增职务
        </el-button>
      </span>
    </el-header>
    ...
    <!-- 弹窗：添加职务 -->
    <el-dialog :title="isAdd ? '添加职务' : '修改职务'" :visible.sync="dialogVisible" width="30%" custom-class="ofa-dialog">
...
// 岗位职务
export default {
  name: PERSON_SETTING_JOB_DUTY.name,
  data() {
    return {
      loading: false, // 加载中
      isAdd: false, // 是否添加模式
      ...
      entity: {},
      validationRule: {
        Name: [{ required: true, message: '请先填写职务名称' }, { min: 2, max: 20, message: '职务名称长度在2~20之间' }],
  ...
  computed: {
    permissions() {
      return this.$root.getPermissions(PERSON_SETTING_JOB_DUTY.name)
    }
  },
  ...
  methods: {
    ...
    del(entity) {
      this.$confirm('确认删除当前职务？', '温馨提示', {
        type: 'warning',
        cancelButtonText: '放弃操作'
      }).then(() => {
        const url = this.$root.getApi(API.KEY, API.JOB_DUTY.BATCH_DELETE)
