<template>
  <el-container v-loading="loading" class="ofa-container column">
    <el-header class="header">
      <span>
        <span class="title">岗位职级<span class="sub-title" style="margin-left: 10px;">共有<label class="ofa-text-primary"> {{list.length}} </label>条数据</span></span>
      </span>
      <span>
        <el-button @click="showAddDialog" type="primary" size="small">
          <font-awesome-icon fas icon="plus"></font-awesome-icon>&nbsp;新增职级
        </el-button>
      </span>
    </el-header>
    <el-main class="content">
      ...
      <el-table :data="list" class="ofa-table" size="small">
        ...
        <el-table-column label="职级名称" prop="Name"></el-table-column>
    ...
    <!-- 弹窗：添加职级 -->
    <el-dialog :title="isAdd ? '添加职级' : '修改职级'" :visible.sync="dialogVisible" width="30%" custom-class="ofa-dialog">
      <el-form :model="entity" :rules="validationRule" ref="typeForm" label-width="120px">
        <el-form-item prop="Name" label="职级名称:">
          <el-input v-model.trim="entity.Name" size="small" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item prop="BaseSalary" label="基本工资:">
          <el-input v-model="entity.BaseSalary" size="small" placeholder="请输入基本工资"></el-input>
        </el-form-item>
...
// 岗位职级
export default {
  name: PERSON_SETTING_JOB_LEVEL.name,
  data() {
    return {
      loading: false, // 加载中
      isAdd: false, // 是否添加模式
      ...
      entity: {},
      validationRule: {
        Name: [{ required: true, message: '请先填写职级名称' }, { min: 2, max: 20, message: '职级名称长度在2~20之间' }],
  ...
  computed: {
    permissions() {
      return this.$root.getPermissions(PERSON_SETTING_JOB_LEVEL.name)
    }
  },
  ...
  methods: {
    ...
    del(entity) {
      this.$confirm('确认删除当前职级？', '温馨提示', {
        type: 'warning',
        cancelButtonText: '放弃操作'
      }).then(() => {
        const url = this.$root.getApi(API.KEY, API.JOB_LEVEL.BATCH_DELETE)
