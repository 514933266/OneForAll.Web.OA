  ...
  <el-container v-loading="loading" class="ofa-container column">
    <el-header class="header">
      <el-row type="flex" justify="space-between">
        <div class="title ofa-text-title">{{ entity.Name + '的个人档案' }}</div>
    ...
    <el-main class="content">
      <div class="person-box">
        <el-avatar :src="(headerDomain + entity.IconUrl)" fit="contain" class="person-icon">
        ...
        <div class="person-workage">
          <label class="ofa-text-grey">已在公司工作了{{ Math.round(entity.JoinAge * 365) }}天</label>
        </div>
      </div>
          ...
          <el-form status-icon :model="fieldSetting" :disabled="disabled" class="form" label-width="140px"
            ...
            <div class="field-box">
              <el-form-item v-for="(field, fIndex) in fieldSetting.Fields" :key="field.Id"
                :rules="[{ required: field.IsRequired, message: `请输入${field.Text}`, trigger: 'blur' }]" :prop="'Fields.' + fIndex + '.Value'">
                <template slot="label">
                ...
                <div v-show="!disabled">
                  <el-input v-if="field.Type === 0" v-model="field.Value" :placeholder="field.Placeholder" size="small"></el-input>
                  <el-date-picker v-if="field.Type === 3" v-model="field.Value" type="date"
                    :placeholder="field.Placeholder" value-format="yyyy-MM-dd" size="small">
                  </el-date-picker>
                  <el-select v-if="field.Type === 5" :placeholder="field.Placeholder" v-model="field.Value" size="small">
                  ...
                  <el-radio-group v-if="field.Type === 6" v-model="field.Value" size="small">
                  ...
                  <el-upload v-if="field.Type === 7" action='#' list-type="picture" :show-file-list="false"
                    :http-request="uploadFile" :data="field" :before-upload="beforeFileUpload" size="small">
                    <el-button>选择文件</el-button>
                  </el-upload>
                  <el-tooltip class="item" effect="dark" :content="field.Placeholder ? field.Placeholder : field.Text"
                    placement="top">
                    <el-checkbox v-if="field.Type === 8" v-model="field.Value" true-label="true" false-label="false" size="small">
                    </el-checkbox>
                  </el-tooltip>
                </div>
              </el-form-item>
            ...
            <div v-if="fieldSetting.IsGrouped">
              <div v-for="(group, gIndex) in fieldSetting.Groups" :key="group.id" class="field-box">
                ...
                <el-form-item v-for="(field, fIndex) in group.Fields" :key="field.Id"
                :rules="[{ required: field.IsRequired, message: `请输入${field.Text}`, trigger: 'blur' }]" :prop="'Fields.' + fIndex + '.Value'">
                <template slot="label">
                  <label>{{field.Text}}</label>
                  <el-tooltip class="item" effect="dark" :content="field.Tips" placement="top">
                ...
                <div v-show="!disabled" class="group-del-button">
                  <el-button @click="removeSettingGroup(fieldSetting, gIndex)" class="ofa-text-danger" type="text" size="small">
...
import { PERSON, PERSON_FORM } from '../../../router/oa-router'
...
export default {
  ...
  methods: {
    ...
    goBack (params) {
      this.$root.navigate({ ...PERSON, params: params })
    }
  },
  created () {
    this.init()
